...
class ProductRepository {
  ...
  async searchProducts(filters = {}) {
    ...
    const params = [];
    let paramIndex = 1;
    let whereConditions = `WHERE p.is_deleted = FALSE AND p.availability_status = 'in_stock' AND (p.price_inr IS NULL OR p.price_inr >= 1)`;
    ...
    if (searchTerm) {
      ...
      const spellingCorrections = {
        ...
        'zebronics': ['zebronics', 'zepronics', 'zebronix', 'zebronic', 'zeb'],
      ...
      const brandKeywords = {
        ...
        zebronics: ['zebronics', 'zepronics', 'zebronix', 'zeb'],
      ...
      const categoryKeywords = {
        earbuds: ['earbuds', 'ear buds', 'earbud', 'earpods', 'ear pods', 'buds', 'pods', 'truly wireless', 'tws', 'bluetooth earbuds', 'sound bomb'],
        headphones: ['headphones', 'headphone', 'over ear', 'on ear', 'wireless headphones'],
        neckbands: ['neckband', 'neck band', 'neckbands', 'neck bands'],
        wired_earphones: ['wired', 'wired earphones', 'wired earphone', 'earphones with wire', 'aux'],
        earphones: ['earphone', 'earphones', 'ear phones']
      };
      ...
      const categorySearchAliases = {
        ...
        neckbands: ['neckbands'],
        wired_earphones: ['wired_earphones', 'earphones'],
        earphones: ['earphones', 'wired_earphones', 'earbuds', 'headphones', 'neckbands']
      };

      const productSearchAliases = {
        earbuds: ['earbud', 'earbuds', 'buds', 'pod', 'pods', 'tws', 'sound bomb'],
        headphones: ['headphone', 'headphones', 'headset'],
        neckbands: ['neckband', 'neck band', 'yoga', 'raga', 'evolve', 'jumbo', 'escape'],
        wired_earphones: ['wired', 'wire', 'aux'],
        earphones: ['earphone', 'earphones', 'earbud', 'earbuds', 'buds', 'pod', 'pods', 'tws', 'sound bomb', 'headphone', 'headphones', 'headset', 'neckband', 'neck band', 'wired', 'wire', 'aux']
      };
      ...
      if (detectedCategories.length > 0) {
        const expandedCategories = [
          ...new Set(detectedCategories.flatMap(cat => categorySearchAliases[cat] || [cat]))
        ];
        const expandedProductTerms = [
          ...new Set(detectedCategories.flatMap(cat => productSearchAliases[cat] || []))
        ];

        // Category match conditions
        const categoryConditions = expandedCategories.map(cat => {
        ...
        for (const term of expandedProductTerms) {
          searchConditions.push(`(p.product_name ILIKE $${paramIndex} OR p.description ILIKE $${paramIndex})`);
          params.push(`%${term}%`);
          paramIndex++;
        }
    ...
    try {
      ...
      const normalizeRetailerKey = (value = '') =>
        String(value).toLowerCase().replace(/[^a-z0-9]/g, '');

      const retailerMatches = (retailerName, aliases = []) => {
        if (!retailer) return true;
        const requested = normalizeRetailerKey(retailer);
        return [retailerName, ...aliases].some(alias => normalizeRetailerKey(alias) === requested);
      };

      const displayNameFromTable = (tableName) => {
        const knownLabels = {
        ...
        if (knownLabels[tableName]) return knownLabels[tableName];
      ...
      const queryProductTable = async (tableName, retailerName) => {
        if (!/^[a-z0-9_]+$/.test(tableName)) {
          console.warn(`Skipping unsafe product table name: ${tableName}`);
          return;
        }

        try {
          const retailerParamIndex = params.length + 1;
          const query = `
            SELECT 
              p.*,
              $${retailerParamIndex} as retailer_name
            FROM ${tableName} p
            ${whereConditions}
          `;
      ...
      const knownProductTables = [
        { tableName: 'amazon_products', retailerName: 'Amazon' },
        { tableName: 'flipkart_products', retailerName: 'Flipkart' },
        { tableName: 'samsung_products', retailerName: 'Samsung' },
        { tableName: 'sony_products', retailerName: 'Sony' },
        { tableName: 'croma_products', retailerName: 'Croma' },
        { tableName: 'vijay_sales_products', retailerName: 'VijaySales', aliases: ['Vijay Sales'] },
      ...
      const dynamicTablesResult = await db.query(`
        SELECT table_name
      ...
      for (const row of dynamicTablesResult.rows) {
        const tableName = row.table_name;
        if (queriedTables.has(tableName)) continue;
