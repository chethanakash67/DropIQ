...
function ResultsContent() {
    ...
    const [trendingFallback, setTrendingFallback] = useState<Product[]>([]);
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => {
            ...
            const keywords = [...new Set(results.map((p) => toKeyword(p.product_name)))].slice(0, 6);
    ...
    const performSearch = useCallback(async (q: string, sort: string, min: string, max: string, ret: string, isManual = false) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.products.length === 0) {
                    // Try fuzzy spelling fallback, while keeping any active store filter.
                    if (indexLoaded) {
        ...
        setResultsLoading(false);
    }, [currentUser, setCurrentUser, authenticatedFetch, indexLoaded, clientSearch]);
                    ...
                    : products.length === 0 && !suggestionState && !storeRedirectState
                        ? (
                            <div style={{ padding: '40px 0' }}>
                                ...
                                {trendingFallback.length > 0 && !retailer && (
                                    <>
